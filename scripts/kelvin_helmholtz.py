...
xbasis = d3.RealFourier(coords['x'], size = PARAMS['Nx'], bounds=(0,PARAMS['Lx']), dealias=PARAMS['dealias'])
ybasis = d3.RealFourier(coords['y'], size = PARAMS['Ny'], bounds = (0,PARAMS['Ly']), dealias=PARAMS['dealias'])
print(xbasis)
...
def density(ys):
    ...
    for el in ys:
        if el < 0.25:
            out.append(PARAMS['rho_1'] - rho_m*np.exp((el-0.25)/PARAMS['L']))
        elif 0.25 <= el < 0.5:
            out.append(PARAMS['rho_2'] + rho_m*np.exp((-el+0.25)/PARAMS['L']))
        elif 0.5 <= el < 0.75:
            out.append(PARAMS['rho_2'] + rho_m*np.exp(-(0.75 - el)/PARAMS['L']))
        else:
            out.append(PARAMS['rho_1'] - rho_m*np.exp(-(el-0.75)/PARAMS['L']))
    return out
...
# CFL
CFL = d3.CFL(solver, initial_dt=PARAMS['max_timestep'], cadence=10, safety=0.2, threshold=0.1,
             max_change=1.5, min_change=0.5, max_dt=PARAMS['max_timestep'])
CFL.add_velocity(u)
